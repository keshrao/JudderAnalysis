...
for subs = 1:length(subjectIDs)
    ...
    for mode = 1:3  %normal, masking, judder
        ...
        for di = 1:3
            ...
            accelprof =  nan(length(trlTS), 500); % sufficiently many columns
            ...
            for trli = 1:length(trlTS)
                ...
                thisaccel = diff(thisspeed)./scaling;

                
                % store velocity profile and corresponding time stamps
                accelprof(trli,1:length(thisaccel)) = thisaccel;
                timeprof(trli, 1:length(thisaccel)) = thisdata(starti:idxEnd(trli)-2,1);
            ...
            for trli = 1:length(trlTS)
                ...
                thismaxval(trli) = nanmax(accelprof(trli,1:numIdx1Sec));
            end
            
            
            % some sanity checks
            thismaxval(thismaxval > 500) = [];
            ...
            meanprof = nanmean(accelprof,1);
            stdprof = nanstd(accelprof,1);
            ...
            plot([nanmean(trldurs) nanmean(trldurs)]./1000, [0 max(meanprof(1:numIdx1Sec))], colors{diffi+1}, 'LineWidth', 2)
